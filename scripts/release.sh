...
VERSION="$(grep -E '^appVersion=' "$PROJECT_DIR/gradle.properties" | cut -d= -f2)"
...
echo ""
echo "Next steps:"
echo "  1. Create GitHub Release (include checksums.txt):"
echo "     gh release create v$VERSION $DIST_DIR/$TARBALL_NAME $DIST_DIR/checksums.txt --title 'Noetic v$VERSION'"
echo ""
echo "  2. Tag and push (CI disabled; release is manual):"
echo "     git tag v$VERSION && git push origin v$VERSION"
echo ""
echo "  3. Update Homebrew tap (use --macos-only for macos-only releases):"
echo "     ./scripts/update-formula.sh $VERSION --macos-only"
echo ""
